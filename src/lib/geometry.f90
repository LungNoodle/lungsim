...
!*get_four_nodes*
  subroutine get_four_nodes(ne,string)
    ...
    integer :: ibeg,iend,i_ss_end,nn,np_global,np
    character(len=40) :: sub_string
...
!!! calculates the connectivity of elements in each xi direction
    ...
    if(allocated(elem_cnct_2d))  deallocate (elem_cnct_2d)
    allocate(elem_cnct_2d(-2:2,0:10,num_elems_2d))
    if(allocated(elems_at_node_2d)) deallocate (elems_at_node_2d)
...
!!! before setting up arrays, count the number of lines required
       ...
       allocate(arclength(3,num_lines_2d)) 
       lines_in_elem=0
       lines_2d=0
       nodes_in_line=0
       line_versn_2d=0
